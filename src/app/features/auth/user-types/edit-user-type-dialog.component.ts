...
@Component({
  selector: 'app-edit-user-type-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  template: `
    <h2 mat-dialog-title>Editar Tipo de Usuario</h2>
    <mat-dialog-content>
...
export class EditUserTypeDialogComponent {
  private fb = inject(FormBuilder);
  private dialogRef = inject(MatDialogRef<EditUserTypeDialogComponent>);
  private data = inject(MAT_DIALOG_DATA) as { userType: UserType };
  ...
  constructor() {
    ...
    this.userTypeForm = this.fb.group({
      name: [userType?.name || '', [Validators.required, Validators.minLength(2)]],
      description: [userType?.description || '', [Validators.required, Validators.minLength(5)]],
